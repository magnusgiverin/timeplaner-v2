...
export default function SemesterPlanEditor({
        ...
        originalSemesterPlans.forEach((plan) => {
            plan.events.forEach((event) => {
                ...
                if (!map[plan.courseid][key]) {
                    ...
                    map[plan.courseid][key].studentgroups.push(...(event.studentgroups ?? []));
                }
            });
        });
    ...
    const [activeAids, setActiveAids] = useState<Record<string, Record<string, boolean>>>({});

    useEffect(() => {
        const updateState = () => {
            // Run only on client
            let initialState: Record<string, Record<string, boolean>> = Object.fromEntries(
    ...
    useEffect(() => {
        ...
        if (typeof window !== "undefined") {
            ...
            window.history.replaceState(null, "", `${window.location.pathname}?${existingParams.toString()}`);
        }
        ...
        <div className="w-full flex flex-col gap-4 items-center justify-center">
            ...
            <section className="w-full">
                ...
                <div className="flex flex-col gap-12">
                    {Object.entries(groupedByCourse).map(([courseid, aids]) => (
                        <div key={courseid}>
                            <h3 className="text-xl font-semibold font-noto-sans text-deep-dark mb-2">{courseid}:</h3>
                            <div className="flex flex-row flex-no-wrap overflow-x-auto md:grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                                {Object.entries(aids).map(([key, evData]) => (
                                    <div
                                        key={key}
                                        className={`relative min-w-[350px] md:min-w-none rounded-xl cursor-pointer transition-all ${activeAids[courseid]?.[key]
                                    ...
                                    >
                                        ...
                                        <div className="text-sm p-4">
                                            <p className="font-semibold flex items-center gap-2 mb-2">
                                                <span className="material-icons text-burnt-peach">event_note</span>
                                                {evData.summary}
                                            </p>
                                            <div className="flex items-center gap-2 mb-1">
                                                <span className="material-icons text-burnt-peach">calendar_today</span>
