    ...
    return (
        ...
        <div className="p-4 rounded-xl bg-powder-petal min-w-[300px] md:min-w-[400px] max-w-xl flex flex-col gap-2">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <p>
                        ...
                        {event.staffNames?.join(", ") ||
                            event.staffs?.map((s) => `${s.firstname} ${s.lastname}`).join(", ")}
                    </p>
                </div>
            ) : null}
            ...
            <div className="flex items-center gap-2">
                ...
                <p>
                    Type: {event["teaching-method-name"]} • {event.coursetype} • {event.compulsory ? "Obligatorisk" : "Valgfritt"}
                </p>
            </div>
            ...
            <div className="flex items-center gap-2">
                ...
                <p>
                    {event.status_plenary ? "Plenumsundervisning" : "Ikke plenumsundervisning"} •{" "}
                ...
                <div className="flex flex-row gap-1">
                    ...
                    {event.room.map((r) => (
                        <p key={r.id} className="truncate">
                            {r.roomname} ({r.roomid}) - {r.buildingname} ({r.buildingacronym})
                            {r.videolink ? " • Has Video" : ""} • Campus: {r.campusid}
                        </p>
                    ))}
                </div>
            )}
            ...
            <div className="flex items-center gap-2">
                ...
                <p>Status: {event.status} • Term: {event.terminnr} • LopeNr: {event.lopenr}</p>
            </div>
