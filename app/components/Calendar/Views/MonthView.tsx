        ...
        {weeks.map((week, wIdx) =>
          week.map((day, dIdx) => {
            ...
            return (
              <div
                key={`${wIdx}-${dIdx}`}
                className={`cursor-pointer relative h-32 border-b border-r border-burnt-peach p-2 text-sm ${
                  isCurrentMonth ? "bg-powder-petal " : "bg-rosy-taupe"
                }`}
                onClick={() => {
              ...
              >
                ...
                <div className="mt-1 space-y-1 overflow-hidden">
                  {dayEvents.slice(0, 3).map((ev) => (
                    ...
                    >
                      {JSON.parse(decodeURIComponent(aliasParam))[ev.courseid]}{" "}
                      ({ev.courseid})
                    </div>
                  ))}
                  {dayEvents.length > 3 && (
